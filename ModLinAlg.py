...
def invSVDHermitianCut(A,Th=1e-5):


    try:
        u,s,v=np.linalg.svd(np.float64(A))#+np.random.randn(*A.shape)*(1e-6*A.max()))
    except:
        Name="errSVDArray_%i"%int(np.random.rand(1)[0]*10000)
        print(ModColor.Str("Problem inverting Matrix, saving as %s"%Name))
        print(ModColor.Str("  will make it svd-able"))
...
# def invSVDHermitianCut(A,Th=1e-3):
#     u,s,v=np.linalg.svd(np.float64(A),hermitian=True)
#     s0=s.copy()
#     ind=np.where(s>Th*s.max())[0]
#     ind=np.where(s>0)[0]
#     ssq=1./s[ind]
#     us=u[:,ind]*ssq.reshape(1,ssq.size)
#     Asq=np.conj(np.dot(us,us.T))
#     stop
#     return Asq
